...
toks_cooee <- tokens(cooee_corpus, 
                    ...
                    remove_numbers = TRUE, 
                    remove_symbol = TRUE)
toks_cooee <- tokens_remove(toks_cooee, 
...
```{r}
tmod_lda <- textmodel_lda(dfmat_cooee, k = 8)
...
cooee_df <- data.frame(Id, Letters, Date, Period, Topics) %>%
  ...
  dplyr::ungroup() %>%
  dplyr::mutate(Topics = dplyr::case_when(Topics == "topic1" ~ "Topic1_family",
                                          Topics == "topic2" ~ "Topic2_exploration",
                                          Topics == "topic3" ~ "Topic3_innerself",
                                          Topics == "topic4" ~ "Topic4_landscape",
                                          Topics == "topic5" ~ "Topic5_indiginous",
                                          Topics == "topic6" ~ "Topic6_employment",
                                          Topics == "topic7" ~ "Topic7_journey",
                                          Topics == "topic8" ~ "Topic8_arrival")) %>%
  tidyr::drop_na() %>%
...
cooee_df %>%
  ...
  scale_y_continuous(name = "Proportion (%)", 
                   ...
                   labels = seq(0, 100, 25), 
                   limits = c(0, 105))
ggsave(file = here::here("images", "topic_cooee_gray.png"), 
